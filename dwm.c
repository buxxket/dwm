...
#define SYSTRAY_SPACING 3

typedef struct SystrayIcon {
  Window win;
  struct SystrayIcon *next;
  int mapped;
  int w, h;
} SystrayIcon;
...
enum {
  ...
  NetClientList,
  NetLast
}; /* EWMH atoms (extended window manager hints) */
enum {
  WMProtocols,
  WMDelete,
  WMState,
...
typedef struct {       /* mouse button interactions */
  ...
  void (*func)(
      const Arg *arg); /* a pointer to the function that should be executed */
...
struct Client {     /* a window that dwm is managing */
  ...
  Client *swallowing; /* points to the client this one is swallowing (swallow
                         patch) */
...
static void (*handler[LASTEvent])(XEvent *) = {
    /* maps X event type to matching function */
    [ButtonPress] = buttonpress, /* mouse button */
    [ClientMessage] =
        clientmessage, /* messages sent from windows, like fullscreen toggle */
    [ConfigureRequest] =
        configurerequest, /* window (client) asks to change geometry */
    [ConfigureNotify] = configurenotify, /* root window geom changes */
    [DestroyNotify] =
        destroynotify,           /* window closed - removes client from dwm */
...
static Window systraywin = 0;
static Atom netatom_systray;
static unsigned long systrayorientation = 0;
...
static const Layout *pertag_layouts[NUMTAGS];
static unsigned int pertag_cur = 0;
...
void arrange(Monitor *m) /* if a specific monitor *m is given, it arranges that
                            one. if NULL, it loops thru all */
{
  ...
  if (m) { /* if *m is a single monitor */
    arrangemon(
        m);     /* applies the current layout to windows (tile, spiral, etc)*/
    ...
    XSync(dpy,
          False); /* ensure X server processed all commands (cleaning up) */
...
void arrangemon(
    Monitor *m) { /* applies current layout to window on monitor *m */
  strncpy(m->ltsymbol, m->lt[m->sellt]->symbol,
          sizeof m->ltsymbol);  /* copies layout symbol string to show in bar */
  if (m->lt[m->sellt]->arrange) /* if the layout has an associated arrange func,
                                   call it (ex. tile() )*/
...
void buttonpress(XEvent *e) { /* handles clickable areas on the bar */
  ...
  if (ev->window == selmon->barwin) {
    ...
    if (i < LENGTH(tags) && selmon->showtags) {
      ...
      for (text = s = stext; *s && x <= ev->x;
           s++) { /* loop through to determine which block was clicked */
        if ((unsigned char)(*s) <
            ' ') { /* sig delim, if block boundaries are off check this */
          ch = *s; /* measure width */
          *s = '\0';
...
void cleanup(void) {  /* called when dwm exits normally or thru restart */
  Arg a = {.ui = ~0}; /* switches to viewing all windows before shutdown */
  Layout foo = {
      "", NULL}; /* switches layout to NULL so no more layout logic is ran */
  Monitor *m;
  size_t i;
  ...
  XSetInputFocus(dpy, PointerRoot, RevertToPointerRoot,
                 CurrentTime); /* reverts keyboard focus to root win */
...
void cleanupmon(Monitor *mon) {
  ...
  XDestroyWindow(dpy,
                 mon->barwin); /* deletes bar window from X server entirely */
...
void clientmessage(XEvent *e) {
  ...
  // Systray handling
  if (e->xclient.message_type == netatom_systray) {
    if (e->xclient.data.l[1] == SYSTEM_TRAY_REQUEST_DOCK) {
      Window win = e->xclient.data.l[2];
      // Manage dock request: reparent, map, add to systray list, etc.
      // For now, just map it so you see it's working:
      XMapRaised(dpy, win);
      XReparentWindow(dpy, win, systraywin, 0, 0);
      // TODO: Add management to your systray icon list for layout
      drawbar(selmon);
    }
    return;
  }
...
/* --- SYSTRAY PATCH: SUPPORT CODE --- */
// static long getwindowprop(Window w, Atom atom) {
//   Atom actual;
//   int format;
//   unsigned long n, extra;
//   unsigned char *p = NULL;
//   long ret = 0;
//   if (XGetWindowProperty(dpy, w, atom, 0L, sizeof(long), False,
//   AnyPropertyType,
//                          &actual, &format, &n, &extra, &p) == Success &&
//       p) {
//     ret = *(long *)p;
//     XFree(p);
//   }
...
static Monitor *systraytomon(Monitor *m) {
  // Always draw systray on selected monitor (adjust as needed)
  return selmon;
}
...
static unsigned int getsystraywidth(void) {
  unsigned int w = 0;
  SystrayIcon *icon;
  for (icon = systray.icons; icon; icon = icon->next)
    if (icon->mapped)
      w += bh + SYSTRAY_SPACING;
  return w;
}

// static void updatesystrayicongeom(SystrayIcon *icon, int w, int h) {
//   if (!icon)
//     return;
//   icon->w = w;
//   icon->h = h;
...
// static void updatesystrayiconstate(SystrayIcon *icon, XPropertyEvent *ev) {
//   long flags;
//   int code = 0;
//   if (!icon || ev->atom != xembedatom ||
//       !(flags = getwindowprop(icon->win, xembedatom)))
//     return;
...
//   }
//   if (code)
//     drawbar(systraytomon(selmon));
// }

// static void removesystrayicon(SystrayIcon *icon) {
//   SystrayIcon **pp = &systray.icons;
//   for (; *pp && *pp != icon; pp = &(*pp)->next)
//     ;
//   if (*pp)
//     *pp = icon->next;
//   free(icon);
...
void updatesystray(void) {
  ...
  if (!systraywin) {
    ...
    systraywin =
        XCreateWindow(dpy, root, 1, 1, 1, 1, 0, DefaultDepth(dpy, screen),
                      CopyFromParent, DefaultVisual(dpy, screen),
                      CWOverrideRedirect | CWBackPixel | CWEventMask, &wa);
    XMapRaised(dpy, systraywin);
    XSetSelectionOwner(dpy, netatom_systray, systraywin, CurrentTime);
    XChangeProperty(dpy, root, netatom_systray, XA_WINDOW, 32, PropModeReplace,
                    (unsigned char *)&systraywin, 1);
    XChangeProperty(dpy, systraywin, XInternAtom(dpy, "_NET_WM_NAME", False),
                    XInternAtom(dpy, "UTF8_STRING", False), 8, PropModeReplace,
                    (unsigned char *)"systray", 7);
    XChangeProperty(dpy, systraywin,
                    XInternAtom(dpy, "_NET_SYSTEM_TRAY_ORIENTATION", False),
                    XA_CARDINAL, 32, PropModeReplace,
                    (unsigned char *)&systrayorientation, 1);
  }
}
/* --- END SYSTRAY PATCH SUPPORT --- */

/* --- drawbar with systray --- */
void drawbar(Monitor *m) {
  ...
  // Draw systray (draw last, so icons don't overlap tags)
  if (showsystray && m == systraytomon(m)) {
    SystrayIcon *icon;
    unsigned int s_x = m->ww - getsystraywidth();
    for (icon = systray.icons; icon; icon = icon->next) {
      if (!icon->mapped)
        continue;
      XMoveResizeWindow(dpy, icon->win, s_x, m->by, bh, bh);
      s_x += bh + SYSTRAY_SPACING;
    }
  }

  // ...rest of your drawbar code (tags, layout, title, etc.) unchanged...
  for (c = m->clients; c; c = c->next) {
    occ |= c->tags == TAGMASK ? 0 : c->tags;
...
void drawbars(void) { /* loop through every monitor to redraw statusbar */
  Monitor *m;
...
void focus(Client *c) { /* handling when focus shifts to a different client */
  if (!c || !ISVISIBLE(c)) /* if the target client c isn't visible, */
    for (c = selmon->stack; c && !ISVISIBLE(c); c = c->snext)
      ; /* search the stack for the next visible one to focus */
  if (selmon->sel &&
      selmon->sel != c)      /* if there's a selected window that is not c, */
...
void focusin(XEvent *e) { /* when a window receives focus */
...
void incnmaster(const Arg *arg) {
  ...
  pertag_nmasters[pertag_cur] = selmon->nmaster;
  arrange(selmon);
}
...
void keypress(XEvent *e) /* receives a keyboard press XEvent from X11 */
{
  ...
  keysym =
      XKeycodeToKeysym(dpy, (KeyCode)ev->keycode,
                       0); /* translates raw keycode into symbolic keysym */
  for (i = 0; i < LENGTH(keys);
       i++) /* loop thru all binds in keys[] (config.h) */
    if (keysym == keys[i].keysym &&
        CLEANMASK(keys[i].mod) == CLEANMASK(ev->state) &&
        keys[i].func)               /* check that a pointer function exists */
      keys[i].func(&(keys[i].arg)); /* if the pressed key matches a bind, call
                                       the function associated w/ that bind */
}
...
void manage(Window w,
            XWindowAttributes
                *wa) { /* after a window is passed from maprequest, manage
                          determines how to treat it and add it to the stack */
  Client *c, *t = NULL, *term = NULL;
  Window trans = None;
  XWindowChanges wc;

  c = ecalloc(1, sizeof(Client)); /* allocate and initialize a new Client struct
  ...
  if (XGetTransientForHint(dpy, w, &trans) &&
      (t = wintoclient(
           trans))) { /* if transient, it inherits tags+mon of parent */
    c->mon = t->mon;
    c->tags = t->tags;
  } else {
  ...
  attachaside(c); /* add to the monitor's client list */
  attachstack(c); /* add it to the stack */
  XChangeProperty(dpy, root, netatom[NetClientList], XA_WINDOW, 32,
                  PropModeAppend, /* add to X11 client list */
                  (unsigned char *)&(c->win), 1);
  XMoveResizeWindow(dpy, c->win, c->x + 2 * sw, c->y, c->w,
                    c->h); /* some windows require this */
  setclientstate(c, NormalState);
  if (selmon->sel && selmon->sel->isfullscreen &&
      !c->isfloating) /* if a fullscreen window was focused, toggle fullscreen
                       */
    setfullscreen(selmon->sel, 0);
  if (c->mon == selmon)
    unfocus(selmon->sel,
            0); /* unfocus other monitor if new window is on current monitor */
  c->mon->sel = c;
  arrange(c->mon); /* recalc based on layout */
  XMapWindow(dpy, c->win);
  if (term)
    swallow(term, c); /* if new window is child of a terminal, replace terminal
                         (swallow) */
  focus(NULL);        /* focus the client */
}

void mappingnotify(XEvent *e) { /* keyboard mapping change */
...
void maprequest(XEvent *e) {   /* window requests map */
...
void run(void) {
  ...
  while (running &&
         !XNextEvent(dpy, &ev)) /* keep running while dwm is supposed to be
                                   running, and keep handling events from X */
...
void setlayout(const Arg *arg) {
  ...
  pertag_layouts[pertag_cur] = selmon->lt[selmon->sellt];
...
void setmfact(const Arg *arg) {
  ...
  if (f < 0.05 || f > 0.95)
    return;
  selmon->mfact = f;
  pertag_mfacts[pertag_cur] = selmon->mfact;
  arrange(selmon);
}
...
void setup(void) {
  ...
  screen = DefaultScreen(
      dpy); /* gets the default screen number from the display (dpy) */
  ...
  drw = drw_create(
      dpy, screen, root, sw,
      sh); /* a drawing context for rendering the UI (like the bar) */
  if (!drw_fontset_create(drw, fonts, LENGTH(fonts)))
    die("no fonts could be loaded.");
  lrpad = drw->fonts->h; /* sets lrpad to the height of the font, for correct
                            spacing in bar */
  bh = drw->fonts->h + 2;
  updategeom(); /* setting up monitor geometry, if using multiple monitors,
                   creates a linked list of monitor structs */
  /* init X11 atoms */
  /* this is asking the X server for an atom (a unique integer) corresponding to
   * each string */
  ...
  xembedatom = XInternAtom(dpy, "_XEMBED_INFO", False);
  netatom_systray = XInternAtom(dpy, "_NET_SYSTEM_TRAY_S0", False);
  // Atom orientation = XInternAtom(dpy, "_NET_SYSTEM_TRAY_ORIENTATION", False);
  systrayorientation = 0; // 0 = HORIZONTAL
  /* init cursors */
  ...
  for (i = 0; i < LENGTH(colors); i++)
    scheme[i] = drw_scm_create(
        drw, colors[i],
        3); /* each scheme[i] corresponds to a SchemeNorm, SchemeSel, etc */
 /* init bars */
  updatebars();   /* creates a bar window for each monitor */
  updatestatus(); /* renders the status text */
  updatesystray();

  /* --- initialize pertag arrays --- */
  for (i = 0; i < NUMTAGS; i++) {
    pertag_mfacts[i] = mfact;
  ...
  wmcheckwin = XCreateSimpleWindow(
      dpy, root, 0, 0, 1, 1, 0, 0,
      0); /* a dummy window created so applications can detect the WM */
  /* setting X11 properties so applications know that dwm is the window manager,
   * and supports EWMH hints */
  ...
  wa.event_mask = SubstructureRedirectMask | SubstructureNotifyMask |
                  ...
                  PropertyChangeMask; /* X11 events that dwm will listen for */
  XChangeWindowAttributes(
      dpy, root, CWEventMask | CWCursor,
      &wa); /* applies the event mask and cursor to the root window */
...
void spawn(const Arg *arg) { /* runs shell commands, ie. launching programs from
                                keybinds */
  struct sigaction sa;
...
// final window takes remaining area
{
        ...
        for (n = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), n++);
// loop all tiled windows if (n == 0) // if no tiles then exit return;
...
adjust size for client's border } else if (i % 2) { // odd indexed windows get
top half (vertically) resize(c, nx, ny, nw - 2 * c->bw, nh / 2 - m->gap->gappx -
2 * c->bw, 0); // resize to half height minus inner gap ny += nh / 2 +
m->gap->gappx; // move ny to next available vert space nh -= nh / 2 +
m->gap->gappx; // reduce nh accordingly } else { // now deal with even index -->
left half horizontally resize(c, nx, ny, nw / 2 - m->gap->gappx - 2 * c->bw, nh
- 2 * c->bw, 0); // same logic as w/ the odds nx += nw / 2 + m->gap->gappx; nw
-= nw / 2 + m->gap->gappx;
                }
        }
}
...
void togglebarcolor(const Arg *arg) { /* this really needs to be integrated with
...
void unmanage(Client *c,
              int destroyed) { /* removing a window from dwm's control */
  Monitor *m = c->mon;
  XWindowChanges wc;
...
void view(const Arg *arg) {
  ...
  pertag_cur = newcur;
  selmon->mfact = pertag_mfacts[pertag_cur];
  selmon->nmaster = pertag_nmasters[pertag_cur];
  selmon->lt[selmon->sellt] = (Layout *)pertag_layouts[pertag_cur];
...
Monitor *wintomon(Window w) { /* move window w to monitor m */
  int x, y;
  Client *c;
  Monitor *m;
...
int main(int argc, char *argv[]) {
  ...
  run();  /* main event loop of dwm -->
           * continuously listens to events from the X server (window changes,
           * key presses, mouse)  and sends them to the correct event handler */
  if (restart)
    execvp(argv[0], argv);
  cleanup();
  XCloseDisplay(dpy);
